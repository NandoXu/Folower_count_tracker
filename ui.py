...
def resource_path(relative_path):
    """
    Get absolute path to resource; works for both development and frozen executables.
    """
    try:
...
def load_icon():
    ...
    """
    try:
        # Check if a valid base64 string (i.e. not just the placeholder) is provided.
...
# ------------------ Base64 Data ------------------
# Replace these placeholder strings with your actual valid base64-encoded data when ready.
...
icon_base64 = "".join(icon_base64.split())
...
logo_base64 = "".join(logo_base64.split())
...
class AppUI:
    ...
    LEFT PANEL:
      • Input fields for Name, Link, and Platform.
      • Primary buttons: "Add Account", "Delete".
      • Secondary buttons: "Update Data", "Import CSV", "Export CSV".
    
    RIGHT PANEL:
      • A Treeview table.

    OVERLAY:
      • Displayed during long operations.
    """
    def __init__(self, root, controller):
        ...
        self.icon_img = None   # holds our window icon image reference
        self.logo_img = None   # holds our overlay logo image reference
        ...
        # --- Set the window icon using base64 (if available) or fallback ICO ---
        ...
        style.configure("TButton",
                        background="#C6E7FF", foreground="#333",
        ...
        style.configure("Treeview.Heading",
                        background="#C6E7FF", foreground="#333", font=("Helvetica", 11, "bold"))
        ...
        cb = ttk.Combobox(lf, textvariable=self.platform_var,
                          values=["instagram", "tiktok", "twitter"], state="readonly")
        cb.grid(row=2, column=1, sticky="ew", pady=5)
        cb.current(0)

        # Primary buttons
        ttk.Button(lf, text="Add Account", command=self.on_add).grid(
            row=3, column=0, columnspan=2, sticky="ew", pady=5)
        ttk.Button(lf, text="Delete", command=self.on_delete).grid(
        ...
        # Secondary buttons
        ttk.Button(lf, text="Update Data", command=self.on_update_all).grid(
            row=6, column=0, columnspan=2, sticky="ew", pady=5)
        ttk.Button(lf, text="Import CSV", command=self.on_import_csv).grid(
            row=7, column=0, columnspan=2, sticky="ew", pady=5)
        ttk.Button(lf, text="Export CSV", command=self.ctrl.export_csv).grid(
        ...
        ttk.Scrollbar(rf, orient="vertical", command=self.tree.yview).grid(
            row=0, column=1, sticky="ns")
        self.tree.configure(yscrollcommand=lambda f, s: None)

        self.refresh()

    # ------------------ Spinner Overlay Methods ------------------
    def _spin(self):
    ...
    def show_overlay(self, text="Loading..."):
        ...
        lbl.pack(pady=(0, 10))
        try:
            # Load the overlay logo using logo_base64.
    ...
    # ------------------ Input and Action Methods ------------------
    def clear_inputs(self):
        """Clears the input fields and resets Platform to default."""
        self.name_var.set("")
        self.link_var.set("")
    ...
    def on_add(self):
        ...
        def task():
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Add Error", str(e)))
            finally:
    ...
    def on_delete(self):
        ...
        for sel in selections:
            try:
                ...
                print("Error retrieving tree item:", ex)
        ...
        if path:
            ...
            def task():
                try:
                    ...
                    self.root.after(0, lambda: messagebox.showerror("Import CSV Error", str(e)))
                finally:
                    self.root.after(0, self.hide_overlay)
                    self.root.after(0, self.refresh)
    ...
    def refresh(self):
        ...
        for row in self.ctrl.fetch_all():
            tags = ()
            if str(row[4]).lower() == "failed":
                tags = ("failed",)
            self.tree.insert("", "end", values=row, tags=tags)
        self.tree.tag_configure("failed", background="#FFCCCC")


# ------------------- Dummy Controller with Scrape Queue -------------------
class DummyController:
    def __init__(self):
        self.data = []
        self.scrape_queue = []
        self.scrape_thread_started = False
        # Callback for UI refresh (set by main)
        self.on_data_update = lambda: None

    def add_account(self, name, link, platform):
        # Immediately add account (no scraping on add)
        self.data.append((name, link, platform, 0, ""))

    def delete_account(self, link):
        self.data = [r for r in self.data if r[1] != link]

    def update_all(self):
        # Re-scrape every account (simulate with delay)
        new_data = []
    ...
    def import_csv(self, path):
        with open(path, newline='', encoding="utf-8") as f:
            ...
            for row in reader:
                if len(row) >= 3:
                    account = (row[0], row[1], row[2])
                    self.scrape_queue.append(account)
    ...
    def scrape_account(self, account):
        ...
        if "fail" in link.lower():
            ...
            return (name, link, platform, 12345, "micro")
...
if __name__ == '__main__':
    ...
    # Set up the callback so that continuous scraping triggers UI refresh.
    controller.on_data_update = lambda: root.after(0, app.refresh)
    root.mainloop()